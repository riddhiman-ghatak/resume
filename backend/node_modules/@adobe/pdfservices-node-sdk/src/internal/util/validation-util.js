...
function validateFileRef(sourceFileRef) {
	...
	switch (fileInfo.inputType) {
		...
		case inputTypes.stream:
			...
			break;
		case inputTypes.url:
			throw new Error('Input file can not be sourced from a URL');
			break;
	}
	validateMediaType(fileInfo.mediaType);
}

function validateOutputType(allowedOutputType, outputType) {
	if (!allowedOutputType.includes(outputType)) {
		throw new Error(`OutputType cannot be set to : ${
			outputType}`);
	}
}
...
function validateSealOptions(sealOptions) {

	const cscCredential = _.cloneDeep(sealOptions.getCertificateCredentials()),
		fieldOptions = _.cloneDeep(sealOptions.getFieldOptions());

	validateOperationOptions(sealOptions);
	validateOperationOptions(fieldOptions);
	validateOperationOptions(cscCredential);
}
