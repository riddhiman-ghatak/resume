...
class CreatePDFOperation {
	...
	validate(context) {
		...
		if (!this.sourceFileRef?.input?.inputUrl) {
			...
			if (this.options && !(this.options instanceof CreatePDFFromHtmlOptions)) {
				throw new Error('Invalid option instance type provided for creating PDF from URL');
			}
		}
